function [overall_accuracy,kappa_accuracy,aua,ua,errorMatrix,pa,apa] = calc_error( trueLabelling, segLabelling, labels )
...
ua = (diagVector./(nrPixelsPerClass));
ua(isnan(ua)) = 0;
aua = mean(ua);
...
pa_number = sum(errorMatrix,1);
pa = (diagVector./(pa_number'));
pa(isnan(pa)) = 0;
apa = mean(pa);

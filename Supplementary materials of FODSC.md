...
**To further verify the effectiveness and superiority of the proposed FODSC method, we conduct the Wilcoxon signed-rank test (with the significance level set to $\alpha$ = 0.05) on three hyperspectral datasets, namely IP220, HH270 and SA224. The results are shown in Table 1. The results are presented in Table 1. The symbols in the table are defined as follows: “+” indicates that FODSC is significantly superior to the comparison method; “≈” indicates that there is no statistically significant difference between FODSC and the comparison method; “−” indicates that FODSC performs worse and “/” is the self- comparison.**
...
**We thank the reviewers for their affirmation of our work. Although this paper has not yet been extended to a journal version, we have carefully considered the valuable opinions put forward by the reviewers. A brief analysis of the computational complexity of the proposed FODSC algorithm is provided as follows:**

**For a hyperspectral image $\boldsymbol{X}$, let the total number of pixels be $N$, and the number of bands be $B$. First, the ERS algorithm is adopted to partition the image into $S$ non-overlapping superpixels, with a computational complexity of $\mathcal{O}(B^2 N + B^3)$. On this basis, $n=DS(n \ll N)$ sample latent features are extracted for each superpixel, corresponding to a time cost of $\mathcal{O}(B^2 N)$. In the iterative optimization stage, the update of the coefficient matrix $\boldsymbol{Z}$ involves matrix multiplication and SVD decomposition, with a complexity of $\mathcal{O}(B^3)$; the adaptive update of the membership matrix $\boldsymbol{U}$ and the cluster center $\boldsymbol{C}$ has a complexity of $\mathcal{O}(KBn)$, where $K$ is the number of subspace partitions. Since the value of $K$ is small, after $T$ iterations, the overall optimization complexity is $\mathcal{O}(T B^3)$. Finally, in the band selection stage, the spectral clustering has a complexity of $\mathcal{O}(B^3)$, and the complexity of selecting bands from the clustering results based on the maximum entropy criterion is $\mathcal{O}(BN+B\log B)$. In summary, the overall computational complexity of the proposed method is $\mathcal{O}(T B^3+B^2 N)$.**
